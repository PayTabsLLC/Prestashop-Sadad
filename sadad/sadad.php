...
class Sadad extends PaymentModule
{
	private $_html = '';
	...
	public function __construct()
	{
		...
		$config = Configuration::getMultiple(array('PAYTABS_ID', 'PAYTABS_PASSWORD', 'PAYTABS_MODE'));

		if (isset($config['PAYTABS_ID']))
			$this->paytabs_id = $config['PAYTABS_ID'];
			
		if (isset($config['PAYTABS_PASSWORD']))
			$this->paytabs_password = $config['PAYTABS_PASSWORD'];
			
		if (isset($config['PAYTABS_MODE']))
			$this->paytabs_mode = $config['PAYTABS_MODE'];

		parent::__construct();

		//$this->displayName = $this->l('SADAD Online Payment');
	...
	{
		if (!Configuration::deleteByName('PAYTABS_ID') || !Configuration::deleteByName('PAYTABS_PASSWORD') || !Configuration::deleteByName('PAYTABS_MODE')
				|| !parent::uninstall())
	...
	private function _postValidation()
	{
		...
		{
			if (!Tools::getValue('paytabs_id'))
				$this->_postErrors[] = $this->l('Merchant ID is required.');
			if (!Tools::getValue('paytabs_password'))
	...
	private function _postProcess()
	{
		if (Tools::isSubmit('btnSubmit'))
		{
			Configuration::updateValue('PAYTABS_ID', Tools::getValue('paytabs_id'));
			Configuration::updateValue('PAYTABS_PASSWORD', Tools::getValue('paytabs_password'));
			Configuration::updateValue('PAYTABS_MODE', Tools::getValue('paytabs_mode'));
		}
	...
	{
		$this->_html .= '<img src="../modules/sadad/sadad.png" style="float:left; margin-right:15px;"><b>'.'</b><br /><br />';
		...
		'<form action="'.Tools::htmlentitiesUTF8($_SERVER['REQUEST_URI']).'" method="post">
			...
			<legend>'.$this->l('Merchant settings').'</legend>
				...
				<table border="0" width="500" cellpadding="0" cellspacing="0" id="form">
					...
					<tr><td width="130" style="height: 35px;">'.$this->l('Email').'</td><td><input type="text" name="paytabs_id" value="'.htmlentities(Tools::getValue('paytabs_id', $this->paytabs_id), ENT_COMPAT, 'UTF-8').'" style="width: 300px;" /></td></tr>
					<tr><td width="160" style="height: 35px;">'.$this->l('Secret Key').'</td><td><input type="text" name="paytabs_password" value="'.htmlentities(Tools::getValue('paytabs_password', $this->paytabs_password), ENT_COMPAT, 'UTF-8').'" style="width: 300px;" /></td></tr>
					<tr><td width="130" style="height: 35px;">'.$this->l('Environment').'</td>
					<td>
					<input type="radio" name="paytabs_mode" value="test" '.(Tools::getValue('paytabs_mode', $this->paytabs_mode) == 'test' ? 'checked':'').'>&nbsp;Test&nbsp;
					<input type="radio" name="paytabs_mode" value="live" '.(Tools::getValue('paytabs_mode', $this->paytabs_mode) == 'live' ? 'checked':'').'>&nbsp;Live
					</td></tr>
